...
static int dw2104_frontend_attach(struct dvb_usb_adapter *d)
{
	...
	d->fe[0] = dvb_attach(ds3000_attach, &dw2104_ds3000_config,
			&d->dev->i2c_adap);
	if (d->fe[0] != NULL) {
		dvb_attach(ts2020_attach, d->fe[0],
...
static int ds3000_frontend_attach(struct dvb_usb_adapter *d)
{
	...
	dvb_attach(ts2020_attach, d->fe[0], &dw2104_ts2020_config,
		&d->dev->i2c_adap);
...
static int su3000_frontend_attach(struct dvb_usb_adapter *d)
{
	...
	d->fe[0] = dvb_attach(ds3000_attach, &su3000_ds3000_config,
					&d->dev->i2c_adap);
	if (d->fe[0] != NULL) {
		if (dvb_attach(ts2020_attach, d->fe[0],
...
static int m88rs2000_frontend_attach(struct dvb_usb_adapter *d)
{
	...
	d->fe[0] = dvb_attach(m88rs2000_attach, &s421_m88rs2000_config,
					&d->dev->i2c_adap);
...
static int US6830_frontend_attach(struct dvb_usb_adapter *d)
{
	...
	d->fe[0] = dvb_attach(m88ds3103_attach, &US6830_ds3103_config,
					&d->dev->i2c_adap);
	if (d->fe[0] == NULL)
		return -EIO;
...
static int US6832_frontend_attach(struct dvb_usb_adapter *d)
{
	...
	d->fe[0] = dvb_attach(m88ds3103_attach, &US6832_ds3103_config,
					&d->dev->i2c_adap);
	if (d->fe[0] == NULL)
		return -EIO;
...
static struct rc_map_table rc_map_dvbsky_table[] = {
	{ 0x0000, KEY_0 },
	{ 0x0001, KEY_1 },
	{ 0x0002, KEY_2 },
	{ 0x0003, KEY_3 },
		...
		const struct usb_device_id *id)
{
	...
	s421 = kzalloc(sizeof(struct dvb_usb_device_properties), GFP_KERNEL);
	if (!s421) {
		kfree(p1100);
		kfree(s660);
		kfree(s480_2);
