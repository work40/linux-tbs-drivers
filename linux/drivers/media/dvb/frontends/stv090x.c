...
static int stv090x_optimize_track(struct stv090x_state *state)
{
	...
	reg = STV090x_READ_DEMOD(state, TMGOBS);
	/* STV090x_GETFIELD_Px(reg, ROLLOFF_STATUS_FIELD); */
...
/* Konstantin Dimitrov <kosio.dimitrov@gmail.com>: add it, because STB6100 driver need the SR info */
static int stv090x_get_frontend(struct dvb_frontend *fe, struct dvb_frontend_parameters *p)
{
	struct stv090x_state *state = fe->demodulator_priv;

	dprintk(FE_DEBUG, 1, "Get params");
	p->u.qpsk.symbol_rate = state->srate;
