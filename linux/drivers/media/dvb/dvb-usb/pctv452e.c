...
static int pctv452e_frontend_attach(struct dvb_usb_adapter *a)
{
	...
	a->fe[0] = dvb_attach(stb0899_attach, &stb0899_config,
						&a->dev->i2c_adap);
	if (!a->fe[0])
		return -ENODEV;
	if ((dvb_attach(lnbp22_attach, a->fe[0],
					&a->dev->i2c_adap)) == 0)
...
static int pctv452e_tuner_attach(struct dvb_usb_adapter *a)
{
	if (!a->fe[0])
		return -ENODEV;
	if (dvb_attach(stb6100_attach, a->fe[0], &stb6100_config,
					&a->dev->i2c_adap) == 0) {
		err("%s failed\n", __func__);
		return -ENODEV;
	}
...
/* module stuff */
static int __init pctv452e_module_init(void)
{
	int result;
	if ((result = usb_register(&pctv452e_usb_driver))) {
...
static void __exit pctv452e_module_exit(void)
{
	/* deregister this driver from the USB subsystem */
	usb_deregister(&pctv452e_usb_driver);
}

module_init(pctv452e_module_init);
module_exit(pctv452e_module_exit);
