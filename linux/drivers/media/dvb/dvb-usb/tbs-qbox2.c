...
{
	if (!dvb_attach(stb6100_attach, adap->fe[0], &qbox2_stb6100_config,
		&adap->dev->i2c_adap))
		return -EIO;
...
static int tbsqbox2_frontend_attach(struct dvb_usb_adapter *d)
{
	...
	if (tbsqbox2_properties.adapter->tuner_attach == &tbsqbox2_earda_tuner_attach) {
		d->fe[0] = dvb_attach(stv090x_attach, &earda_config,
					&d->dev->i2c_adap, STV090x_DEMODULATOR_0);
		if (d->fe[0] != NULL) {
			d->fe[0]->ops.set_voltage = tbsqbox2_set_voltage;
