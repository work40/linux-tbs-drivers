...
struct dvb_frontend *m88rs6000_attach(const struct m88rs6000_config *config,
...
EXPORT_SYMBOL(m88rs6000_attach);
...
static int m88rs6000_set_frontend(struct dvb_frontend *fe, struct dvb_frontend_parameters *p)
{
	struct m88rs6000_state *state = fe->demodulator_priv;
	struct dtv_frontend_properties *c = &fe->dtv_property_cache;
...
{	
	*delay = HZ / 5;
	...
	if (p) {
		int ret = m88rs6000_set_frontend(fe, p);
		if (ret)
			return ret;
	}
